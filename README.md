...
| An entry filled at a type the macro cannot describe -- `char`, `i128`, `u128`, `f16`, `f128` | Refused where `zod` or `jsonschema` is on, spanned on the filling -- those are the two surfaces that read a declared filling. A name the type dispatch has no arm for is read as another `#[model_schema]` item, which is right for a sibling declared below and gibberish for a primitive: the JSON emission would name a `char_schema` module nothing publishes, and the zod `$SchemaDefault` a `char$Schema` binding no generated module exports. Only the primitive names the language reserves are refused, so a forward-referenced sibling keeps compiling. |
